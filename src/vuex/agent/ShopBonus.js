  ...
  actions: {
    ...
    get_list({ state }, obj) {
      ...
      return new Promise((resolve, reject) => {
        ...
        axios
          .get("/api-s/shops/findAllShareProfitTable", { params: obj.query })
          .then(x => {
    ...
    获取商家分润列表({ state }) {
      ...
      axios
        .get("/api-s/shops/sumForAgentProfit", { params: state.商家.query })
        .then(x => {
          if (x.data.code == 200) {
    ...
    获取会员分润列表({ state }) {
      ...
      axios
        .get("/api-s/shops/userSumForAgentProfit", { params: state.会员.query })
        .then(x => {
          if (x.data.code == 200) {
