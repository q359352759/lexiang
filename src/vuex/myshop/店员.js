    ...
    actions:{
        ...
        添加店员({},obj){
            return new Promise((resolve, reject) => {
                axios.post('/api-s/shops/addShopClerks',obj).then(x=>{
                    resolve(x)
                }).catch(err=>{
                    reject(err)
                })
        ...
        查询店员列表({state,rootGetters,dispatch}){
            ...
            return new Promise((resolve, reject) => {
                ...
                state.店员列表.query.start=state.店员列表.page_index*state.店员列表.query.length;
                axios.get('/api-s/shops/findShopClerksAll',{params:state.店员列表.query}).then(x=>{
                    // console.log('查询店员',x)
        ...
        设置员工基本信息({},obj){
            ...
            // dispatch('user/通过username查询用户',obj.clerksid,{root: true}).then(x=>{
            //     console.log(x)
            //     obj.user=x.data.data;
            // })
        },
        ...
        移除员工({state},[item,index]){
            return new Promise((resolve, reject) => {
                axios.get('/api-s/shops/deleteShopClerks?id='+item.id).then(x=>{
                    if(x.data.code==200){
        ...
        查询所有店员({state,rootGetters}){
            ...
            return new Promise((resolve, reject) => {
                axios.get('/api-s/shops/findShopClerksAll',{params:query}).then(x=>{
                    // console.log('查询店员',x)
                    var list=x.data.data.data;
                    state.所有店员=list;
                    resolve()
