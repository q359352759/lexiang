    ...
    actions:{
        ...
        async 添加考勤时间({state,dispatch,rootGetters}){
            ...
            if(state.班次.length==0){
                ...
                await dispatch('查询班次');
            }else if(state.班次.length>0 && state.班次类型!=state.班次[0].schedulingtype){
                var 修改班次对象=Object.assign({},state.班次[0]);
                    修改班次对象.schedulingtype=state.班次类型
                    await dispatch('修改班次',修改班次对象);                   
            }
                var 固定班次不能提交=state.固定班次.find(x=>(x.officehours=='' || x.quittingtime==''))
                if(!固定班次不能提交){
                    try {
                ...
                var 三班不能提交=state.三班.find(x=>(x.officehours=='' || x.quittingtime==''))
                if(!三班不能提交){
        ...
        查询班次({state,dispatch,rootGetters}){
            ...
            var 店铺=rootGetters['get_myshop']
            return new Promise((resolve, reject) => {
                axios.get('/api-s/shops/findShopSchedulingByShopid',{params:{shopid:店铺.shopid}}).then(x=>{
                    console.log('查询班次',x);
                    var 班次=x.data.data || [];
                    state.班次=班次;
                    if(班次.length>0){
