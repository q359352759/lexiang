...
export default {
  ...
  actions: {
    ...
    async 添加考勤时间({ state, dispatch, rootGetters }) {
      ...
      if (!固定班次不能提交) {
        try {
          var 添加单个考勤时间 = await dispatch(
            "添加单个考勤时间",
            state.固定班次[0]
          );
        } catch (error) {}
      }
      var 两班不能提交 = state.两班.find(
        x => x.officehours == "" || x.quittingtime == ""
      );
      if (!两班不能提交) {
    ...
    添加单个考勤时间({ state, rootGetters }, obj) {
      ...
      return new Promise((resolve, reject) => {
        if (obj.id) {
          axios
            .post("/api-s/shops/updateShopShift", obj)
            .then(x => {
              if (x.data.code != 200) {
                mui.alert(
          ...
          axios
            .post("/api-s/shops/addShopShift", obj)
            .then(x => {
              if (x.data.code != 200) {
                mui.alert(
                  x.data.msg ? x.data.msg : x.data.message,
    ...
    查询考勤时间({ state, rootGetters }) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .get("/api-s/shops/findShopShiftByShopid/" + 商家.shopid)
          .then(x => {
            if (x.data.code == 200) {
    ...
    修改班次({ state }, obj) {
      return new Promise((resolve, reject) => {
        axios
          .post("/api-s/shops/updateShopScheduling", obj)
          .then(x => {
            if (x.data.code != 200) {
              mui.alert(
                x.data.msg ? x.data.msg : x.data.message,
    ...
    添加班次({}, obj) {
      return new Promise((resolve, reject) => {
        axios
          .post("/api-s/shops/addShopScheduling", obj)
          .then(x => {
            if (x.data.code != 200) {
              mui.alert(
                x.data.msg ? x.data.msg : x.data.message,
