  ...
  actions: {
    ...
    async updated_user() {
      return new Promise((resolve, reject) => {
        ...
        axios
          .post("/api-u/users/update/headImgUrl", qs.stringify(userInfo))
          .then(x => {
            if (x.data.code == 200) {
            } else {
    ...
    设置openid({}) {
      ...
      axios
        .post("/api-u/users/update/openid", qs.stringify(obj))
        .then(x => {
          console.log("设置微信", x);
        })
        .catch(err => {
    ...
    通过电话号码获取用户({}, phone) {
      return new Promise((resolve, reject) => {
        axios
          .get("/api-u/users/findByPhone?phone=" + phone)
          .then(x => {
            resolve(x);
          })
          .catch(err => {
    ...
    通过username查询用户({}, username) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .get("/api-u/users/findByUserid/" + username)
          .then(x => {
            resolve(x);
          })
          .catch(err => {
