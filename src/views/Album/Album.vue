...
export default {
  ...
  methods: {
    ...
    Sure() {
      if (this.size == 0) {
        ...
        //判读是否有图片url
        if (
          this.Select_picture.length > 0 &&
          typeof this.Select_picture[0] == "string"
        ) {
          var list = [];
          var Select_picture = this.Select_picture;
      ...
      this.$axios({
        ...
        .then(x => {
          ...
          if (x.data.code == 200) {
            ...
            mui.alert(
              x.data.msg ? x.data.msg : x.data.error,
              "提示",
              "我知道了",
              function() {},
              "div"
  ...
  mounted: function() {
    ...
    $(".zhengmianInput").on("change", "input", function() {
      ...
      if (size > 1024) {
        this_1.option.outputSize = size / 1024;
      } else {
        this_1.option.outputSize = 1;
      }
