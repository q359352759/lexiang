...
export default {
  ...
  methods: {
    ...
    addguanggaoji() {
      var this_1 = this;
      var tishi =
        "恭喜您已成为区域代理，成功获得价值498元的广告机套餐，点击公众号内的“广告管理”使用，初始账号为您的手机号，密码为手机号后6位，请在广告机内点击“会员服务”修改密码（请勿在红包乐购中修改）。";
      var tishi1 =
        "恭喜您已成为区域代理，成功获得价值498元的广告机套餐，已经叠加到您的广告机账户。";
      var obj = {
    ...
    get_areaManager() {
      ...
      this.$axios({
        method: "get",
        url:
          "/api-u/areaManager/findByAreaCode?areaCode=" +
          this.city[this.city.length - 1].id
      })
        .then(x => {
          console.log(x);
          if (x.data.code == 200) {
            ...
            this.Apply_type = 2;
            mui.alert(
              "恭喜！该区域代理商尚属空缺，请第一时间申请，不然随时可能被其他代理人申请，机会只有一次！",
              "提示",
              function() {},
              "div"
            );
            ...
            this.Apply_type = 3;
            mui.alert(
              '该区域已有代理商，但你目前还有机会该区域代理商的备选人员，请联系我公司客服进行申请区域代理的具体事项，客服电话：<a href="tel:028-83401283">028-83401283</a>',
              "提示",
              function() {},
              "div"
            );
