  ...
  methods: {
    获取店铺() {
      return new Promise((resolve, reject) => {
        this.$axios
          .get("/api-s/shops/findByShopid/" + this.shopid)
          .then(x => {
            if (x.data.code == 200) {
              this.店铺 = x.data.data;
            } else {
    ...
    获取有效红包() {
      ...
      this.$axios
        .get("/api-s/shops/redenvelope/findAll", { params: this.红包.query })
        .then(x => {
          console.log(x);
          if (x.data.code == 200) {
    ...
    scroll(e) {
      ...
      if (
        h + t >= sh - 10 &&
        !this.红包.loading &&
        this.红包.list.length < this.红包.total
      ) {
        this.红包.page_index++;
        this.获取有效红包();
      }
