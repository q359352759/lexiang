...
export default {
  ...
  methods: {
    ...
    content_scroll(e) {
      ...
      if (h + t >= sh - 10) {
        if (this.type_1 == 1) {
          if (
            !this.redenvelope.loading &&
            this.redenvelope.list.length < this.redenvelope.total
          ) {
    ...
    get_redenvelope() {
      ...
      this.redenvelope.query.start =
        this.redenvelope.page_index * this.redenvelope.query.length;
  ...
  created: function() {
    ...
    if (
      !homeDialog_obj ||
      homeDialog_obj == "" ||
      username != homeDialog_obj.userid
    ) {
