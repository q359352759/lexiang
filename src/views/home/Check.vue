  ...
  methods: {
    ...
    zhifu() {
      ...
      if (this.payment_type == 0) {
        ...
        if (
          this.youwu_hongbao &&
          this.money_zong_dikou &&
          this.money_zong_dikou != 0
        ) {
          //使用了红包
          if (this.dikou_type == 1) {
        ...
        openloading(true);
        this.$request("/api-s/shops/createOrders", obj, "post")
          .then(x => {
            console.log("添加订单", x);
            if (x.data.code == 200) {
              this.$router.push({
        ...
        this.shangpin_dikou.forEach(item => {
          ...
          let obj = {
            ...
            actualPayment: item.dikou
              ? Math.ceil((item.sellingPrice - item.dikou) * 100) / 100
              : item.sellingPrice //单个商品的实际金额
          };
