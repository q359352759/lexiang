...
export default {
    ...
    methods: {
        ...
        reslist(list) {
            ...
            if (this.img_type == 1) {
                ...
                for (var i = 0; i < list.length; i++) {
                    if (list[i].url) {
                        html +=
                            '<div style="margin:0px;"><img style="max-width: 100%;" width="100%" src="' +
                            list[i].url +
                            '" alt=""></div><div style="min-height:20px;"></div>';
                    } else {
                        html +=
                            '<div style="margin:0px;"><img style="max-width: 100%;" width="100%" src="' +
                            list[i] +
        ...
        input_change(x) {
            ...
            if (
                !this.add_obj.sellingPrice ||
                !this.add_obj[x] ||
                !number_test.test(this.add_obj[x])
            ) {
                if (x == "sellingPrice") {
            ...
            if (x == "sellingPrice") {
                ...
                if (
                    this.add_obj.percentage &&
                    number_test.test(this.add_obj.percentage)
                ) {
                    //比例
                    this.add_obj.percentage =
                        Math.floor(parseFloat(this.add_obj.percentage) * 10) / 10;
                    this.add_obj.deduction = (
                        (this.add_obj.percentage * this.add_obj.sellingPrice) /
                        100
                    ).toFixed(2);
                } else if (
                    this.add_obj.deduction &&
                    number_test.test(this.add_obj.deduction)
                ) {
                    this.add_obj.deduction =
                        Math.floor(parseFloat(this.add_obj.deduction) * 100) / 100;
                    this.add_obj.percentage = (
                        (this.add_obj.deduction / this.add_obj.sellingPrice) *
                        100
                    ).toFixed(1);
                }
                ...
                if (
                    this.add_obj.commissionPercentage &&
                    number_test.test(this.add_obj.commissionPercentage)
                ) {
                    this.add_obj.commissionPercentage =
                        Math.floor(parseFloat(this.add_obj.commissionPercentage) * 10) / 10;
                    this.add_obj.commission = (
                        (this.add_obj.commissionPercentage * this.add_obj.sellingPrice) /
                        100
                    ).toFixed(1);
                } else if (
                    this.add_obj.commission &&
                    number_test.test(this.add_obj.commission)
                ) {
                    this.add_obj.commission =
                        Math.floor(parseFloat(this.add_obj.commission) * 100) / 100;
                    this.add_obj.commissionPercentage = (
                        (this.add_obj.commission / this.add_obj.sellingPrice) *
                        100
                    ).toFixed(1);
                }
            }
            ...
            if (x == "percentage") {
                ...
                var deduction =
                    (this.add_obj.percentage / 100) * this.add_obj.sellingPrice;
            ...
            if (x == "deduction") {
                ...
                var percentage =
                    (this.add_obj.deduction / this.add_obj.sellingPrice) * 100;
            ...
            if (x == "commissionPercentage") {
                //百分比
                this.add_obj.commissionPercentage =
                    Math.floor(this.add_obj.commissionPercentage * 10) / 10;
                var commission =
                    (this.add_obj.commissionPercentage / 100) * this.add_obj.sellingPrice;
                this.add_obj.commission = Math.floor(commission * 100) / 100;
            }
            if (x == "commission") {
                //金额
                this.add_obj.commission =
                    Math.floor(this.add_obj.commission * 100) / 100;
                var commissionPercentage =
                    (this.add_obj.commission / this.add_obj.sellingPrice) * 100;
                this.add_obj.commissionPercentage =
                    Math.floor(commissionPercentage * 10) / 10;
            }
        },
        //返回上一页
        back() {
        ...
        add() {
            ...
            if (!this.add_obj.name) {
                ...
                !this.add_obj.sellingPrice ||
                isNaN(this.add_obj.sellingPrice)
            ) {
                ...
                this.是否专享 &&
                this.专享商品.deduction <= this.add_obj.deduction
            ) {
                mui.toast("专享抵扣需大于商品抵扣。", { duration: 2000, type: "div" });
                return;
            } else if (
                this.是否专享 &&
                this.专享商品.deduction > this.add_obj.sellingPrice * 0.9
            ) {
                mui.toast(
                    "抵扣金额不能大于商品金额的90%即" +
                    this.add_obj.sellingPrice * 0.9 +
                    "元。",
            ...
            if (this.是否专享) {
                ...
                this.专享商品.typeName =
                    this.专享商品.type == 0 ? "新人专享" : "生日专享";
                this.专享商品.shopid = this.myshop.shopid;
                this.专享商品.percentage = Math.fround(
                    (this.专享商品.deduction / this.add_obj.sellingPrice) * 100
                );
            }
            // return
        ...
        修改商品和删除专享() {
            openloading(true);
            Promise.all([this.修改商品([this.add_obj]), this.删除专享(this.专享商品)])
                .then(x => {
                    if (x[0].data.code == 200 && x[1].data.code == 200) {
                        mui.toast("设置成功", { duration: "long", type: "div" });
                        history.back();
        ...
        获取专享商品() {
            this.loading = true;
            this.$axios("/api-s/shops/findExclusiveByCommodityId/" + this.add_obj.id)
                .then(x => {
