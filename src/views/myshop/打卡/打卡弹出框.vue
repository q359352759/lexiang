        ...
        async 开始打卡(){
            if(this.员工自己的信息.workstate==0){
                var obj={
                    ...
                    replaceid:(this.替人打卡 && this.选择的替班人员) ? this.选择的替班人员.clerksid : '',   //替班id
                ...
                if(!添加打卡.data.code || 添加打卡.data.code!=200){
                    if(添加打卡.data.msg || 添加打卡.data.message){
                        mui.alert(添加打卡.data.msg ? 添加打卡.data.msg : 添加打卡.data.message, "提示",'我知道了', function() {},"div");                                
                    }else{
                ...
                try {
                    ...
                    if(打下班卡.data.code!=200){
                        mui.alert(打下班卡.data.msg ? 打下班卡.data.msg : 打下班卡.data.message, "提示",'我知道了', function() {},"div");
                        return
                    }
                } catch (error) {
                    mui.toast('网络错误，稍后再试。', {duration: "long", type: "div" });
            ...
            var 修改员工信息=await this.修改员工信息(员工);     
                if(修改员工信息.data.code==200){
                    ...
                    if(修改员工信息.data.msg || 修改员工信息.data.message){
                         mui.alert(修改员工信息.data.msg ? 修改员工信息.data.msg : 修改员工信息.data.message, "提示",'我知道了', function() {},"div");
                    }else{
                        mui.alert('网络错误，稍后再试。', "提示",'我知道了', function() {},"div");
                    }
                }
            this.查询所有店员();
            this.查询自己的打卡记录()
