...
export default {
    ...
    methods: {
        ...
        async 保存(){
            if(this.班次类型==1){
                var 不能提交=this.固定班次.find(x=>(x.officehours=='' || x.quittingtime==''))
                if(不能提交){
                    mui.toast('请选择时间', {duration: "long", type: "div" });
                    return              
                }
            }else if(this.班次类型==2){
                var 不能提交=this.两班.find(x=>(x.officehours=='' || x.quittingtime==''))
                if(不能提交){
                    mui.toast('请选择时间', {duration: "long", type: "div" });
                    return              
                }
            }else if(this.班次类型==3){
                var 不能提交=this.三班.find(x=>(x.officehours=='' || x.quittingtime==''))
                if(不能提交){
                    mui.toast('请选择时间', {duration: "long", type: "div" });
                    return              
                }
