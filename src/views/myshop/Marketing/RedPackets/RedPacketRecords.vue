...
export default {
    ...
    methods:{
        ...
        xiangqing(item,type){
            ...
            if(type){
                this.$router.push('/redPacketsDetails?type='+item.type+'&id='+item.id)
            }else{
                this.$router.push('/redPacketsDetails?type='+item.type+'&id='+item.id+'&shixiao=1')
            }
        },
        //选择类型
        select_type(x){
        ...
        scroll(e) {
            ...
            if (h + t >= sh - 10) {
                console.log("到底底部");
                if(this.type==0 && !this.shengxiao.loading && this.shengxiao.list.length!=this.shengxiao.total){
                    this.shengxiao.page_idnex++;
                    this.get_redenvelope(this.shengxiao)
                }else if(this.type==1 && !this.shixiao.loading && this.shixiao.list.length!=this.shixiao.total){
