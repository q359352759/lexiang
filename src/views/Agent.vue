...
export default {
    ...
    computed: {
        ...
        总可提现() {
            ...
            总可提现 +=
                this.代理人分润资产 && this.代理人分润资产.balance
    ...
    methods: {
        ...
        平台补贴提现() {
            ...
            return new Promise((resolve, reject) => {
                ...
                this.$axios
                    .get("/api-u/users/alipay", { params: obj })
                    .then(x => {
                        resolve(x);
                    })
                    .catch(err => {
            ...
            Promise.all([this.平台补贴提现(), this.店铺分润提现()]).then(res => {
                ...
                if (!res[0]) {
                    ...
                    if (res[0].data.code == 200) {
                        ...
                        res[0].data.code == "PAYEE_USER_INFO_ERROR" ||
                        res[0].data.code == "PAYEE_ACC_OCUPIED"
                    ) {
                        是否需要输入名字 = true;
                    } else {
                        this.提现提示语[0].type = 3;
